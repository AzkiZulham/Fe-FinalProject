...
export default function PeakSeasonCalendar({ roomTypeId }: { roomTypeId: number }) {
  const [events, setEvents] = useState<PeakSeasonEvent[]>([]);
  const [loading, setLoading] = useState(true);
  const [view, setView] = useState<View>("month");
  const [currentDate, setCurrentDate] = useState(new Date());
  const [selectedDates, setSelectedDates] = useState<Date[]>([]);
  const [selectedType, setSelectedType] = useState<'normal' | 'peak' | 'unavailable'>('normal');
  ...
  const fetchData = useCallback(async () => {
    try {
      setLoading(true);
      const res = await axios.get<ApiPeakSeason[]>(`/api/peak-season/${roomTypeId}`);
  ...
  const handleCancel = () => {
    ...
    setAdjustmentValue('');
    setAdjustmentType('percentage');
    setSelectionKey(prev => prev + 1); // Force calendar re-render to reset selection
  };

  const handleSelectEvent = async (event: PeakSeasonEvent) => {
      ...
      try {
        await Promise.all(
          selectedEvents.map(event =>
            event.id ? axios.delete(`/api/peak-season/${event.id}`) : Promise.resolve()
          )
        );
        await fetchData();
        setSelectedEvents([]);
          ...
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-xs sm:text-sm text-gray-600">Klik tanggal untuk atur harga</p>
              </div>
            </div>
        ...
        <div className="p-2 sm:p-4 md:p-6">
          ...
          <div className="mt-4 sm:mt-6 p-3 sm:p-4 bg-gray-50 rounded-lg border border-gray-200">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
              <div>
                <h3 className="text-xs sm:text-sm font-semibold text-gray-900 mb-2 sm:mb-3">Keterangan:</h3>
                <div className="flex flex-wrap gap-3 sm:gap-4">
                  <div className="flex items-center gap-2">
                    <div className="w-2 h-2 sm:w-3 sm:h-3 bg-green-500 rounded"></div>
                    <span className="text-xs sm:text-sm text-gray-700">Harga Normal</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <div className="w-2 h-2 sm:w-3 sm:h-3 bg-amber-500 rounded"></div>
                    <span className="text-xs sm:text-sm text-gray-700">Harga Naik</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <div className="w-2 h-2 sm:w-3 sm:h-3 bg-red-500 rounded"></div>
                    <span className="text-xs sm:text-sm text-gray-700">Tidak Tersedia</span>
                  </div>
                </div>
              </div>
            </div>
              ...
              <div className="flex items-center justify-between mb-4">
                <div>
                  ...
                  <p className="text-xs sm:text-sm text-gray-600 mt-1">
                    ...
                    {selectedDates.length > 1 && ` - ${format(selectedDates[selectedDates.length - 1], "dd MMM yyyy")}`}
                  </p>
                </div>
                <button
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                    ...
                    >
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs sm:text-sm font-medium">Harga Normal</span>
                      </div>
                      <p className="text-xs text-gray-600 mt-1">Tersedia dengan harga biasa</p>
                    </button>
                    ...
                    >
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs sm:text-sm font-medium">Naikkan Harga</span>
                      </div>
                      <p className="text-xs text-gray-600 mt-1">Tambah persentase atau nominal</p>
                    </button>
                    ...
                    >
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs sm:text-sm font-medium">Tidak Tersedia</span>
                      </div>
                      <p className="text-xs text-gray-600 mt-1">Tutup pemesanan</p>
                    </button>
                  </div>
                </div>
                    ...
                    <div className="flex flex-col sm:flex-row gap-3">
                      <div className="flex-1">
                        <select
                          ...
                          onChange={(e) => setAdjustmentType(e.target.value as 'percentage' | 'nominal')}
                      ...
                      <div className="flex-1">
                        <input
                          ...
                          placeholder={adjustmentType === 'percentage' ? '20' : '100000'}
                          className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-sm"
                        />
                      </div>
                    </div>
                    {adjustmentValue && (
                      <p className="text-xs sm:text-sm text-gray-600">
                        {adjustmentType === 'percentage'
                          ...
                          : `Harga akan naik Rp${parseInt(adjustmentValue).toLocaleString('id-ID')}`
                        }
                      </p>
                    )}
                  </div>
