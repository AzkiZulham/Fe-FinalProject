...
import TenantAvatarUpload from "./TenantAvatarUpload";
import TenantPersonalInfoForm from "./TenantPersonalInfoForm";
import TenantSecurityForm from "./TenantSecurityForm";
import { TenantData } from "./TenantTypes";

export default function TenantUpdateProfilePage() {
  const { user } = useAuth();
  ...
  useEffect(() => {
    if (user) {
      setTenantData({
        ...
        gender: user.gender || "",
        profileImg: user.avatar || "/default-avatar.png",
    ...
    <ProtectedPage role="TENANT">
      <main className="min-h-screen bg-[#f8fafc] mb-20">
        <section className="relative bg-gradient-to-r from-[#2f567a] to-[#4c7ba5] py-20 text-white">
          ...
          >
            <TenantAvatarUpload tenantData={tenantData} setTenantData={setTenantData} />
          </motion.div>
        </section>
