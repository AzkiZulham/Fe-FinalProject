...
import { Calendar, Users, CreditCard, Shield, Check, User, Baby, Home, Info, X } from "lucide-react";
...
const BookingSidebar: React.FC<Props> = ({ selectedRoom, onBook, checkInDate, setCheckInDate, checkOutDate, setCheckOutDate }) => {
  ...
  React.useEffect(() => {
    ...
    .then(data => {
      if (data && typeof data.total === 'number' && typeof data.nights === 'number') {
        setPricing(data);
      } else {
  ...
  return (
    <div id="booking-sidebar" className=" rounded-2xl shadow-2xl border border-gray-200/60 p-6 sticky top-6 backdrop-blur-sm bg-white/95">
      <div className="text-center mb-6 pb-4 border-b border-gray-200/60">
        ...
        <p className="text-sm text-gray-500">Dapatkan harga terbaik dengan pemesanan online</p>
      </div>
      ...
      {selectedRoom ? (
        <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-2xl p-5 mb-6 border border-blue-200/60 shadow-sm relative overflow-hidden">
          ...
          <div className="relative z-10">
            <div className="flex items-center justify-between mb-3">
              <h4 className="font-bold text-gray-900 text-lg leading-tight">{selectedRoom.roomName}</h4>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-2 sm:space-y-0">
              ...
              <div className={`text-lg font-bold ${getPriceColor()} flex items-end space-x-1`}>
                {isPeakSeason ? (
                  <div className="flex flex-col items-end">
                    ...
                    <div className="flex items-center">
                      <span>Rp {Math.round(displayPrice).toLocaleString('id-ID')}</span>
                      <span className="text-sm font-normal text-gray-500 ml-1">/malam</span>
                    </div>
                    {isPeakSeason && (
        ...
        <div className="bg-gradient-to-r from-amber-50 to-orange-50 rounded-2xl p-5 mb-6 border border-amber-200/60 shadow-sm">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <p className="text-amber-800 font-medium text-sm">Pilih Tipe Kamar</p>
              <p className="text-amber-600 text-xs mt-1">Silakan pilih tipe kamar terlebih dahulu untuk melanjutkan pemesanan</p>
            </div>
          </div>
        </div>
      )}
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-xl p-3 mb-3 animate-fadeIn">
            <div className="flex justify-between items-start">
              <p className="text-blue-700 text-xs flex-1">
                <strong>Tips:</strong> Pilih tanggal check-in dan check-out untuk melihat ketersediaan kamar dan harga terupdate. 
                Harga dapat berubah selama musim puncak.
              </p>
              <button 
      ...
      {pricing && !pricingLoading && typeof pricing.total === 'number' && typeof pricing.nights === 'number' && (
        <div className="bg-gradient-to-br from-gray-50 to-blue-50 rounded-2xl p-5 mb-6 border border-gray-200/60 shadow-sm">
          ...
          <div className="space-y-3">
            <div className="flex justify-between items-center text-sm">
              <span className="text-gray-600">{qty} Kamar × {pricing.nights} Malam</span>
            ...
            {pricing.normalNights > 0 && (
              <div className="flex justify-between text-sm text-gray-600">
                ...
                <span>Rp {(pricing.normalTotal || 0).toLocaleString('id-ID')}</span>
              </div>
            )}
            ...
            <div className="border-t border-gray-300 pt-3 mt-2">
              <div className="flex justify-between items-center">
                ...
                <span className="text-xl font-bold text-blue-600">Rp {(pricing.total || 0).toLocaleString('id-ID')}</span>
              </div>
      ...
      <button
        ...
        disabled={!selectedRoom || !checkInDate || !checkOutDate || calculateNights() <= 0 || isDateAvailable === false}
      ...
      >
        ...
        <span className="relative z-10">
          {selectedRoom && checkInDate && checkOutDate && isDateAvailable !== false ? 'Pesan Sekarang' : 'Pilih Kamar & Tanggal'}
        </span>
      </button>
        ...
        <div className="fixed inset-0 backdrop-blur-md flex items-center justify-center z-50 p-4 animate-fadeIn">
          <div className="bg-white rounded-2xl p-6 max-w-sm w-full shadow-2xl border border-gray-200 transform animate-scaleIn">
            <div className="text-center mb-4">
              ...
              <p className="text-gray-600 text-sm">Masuk ke akun Anda untuk melanjutkan pemesanan</p>
            </div>
